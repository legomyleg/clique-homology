...
def get_cliques(G:nk.Graph):
    ...
    all_cliques_list: list = sorted(list(all_cliques), key=len)
...
def ranks_and_nullities(M:np.ndarray) -> tuple:
    """
...
def betti_numbers(G, attr:dict|None=None, method:str="clique") -> np.ndarray: # type: ignore
    """
